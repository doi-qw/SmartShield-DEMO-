...
const CONFIG = {
    ...
    TOLERANT_MODE: true  // Добавлен режим толерантности к ошибкам
};
...
class IndustrialParser {
    ...
    parse() {
        ...
        try {
            // ПРЕЖДЕ ЧЕМ ПАРСИТЬ, проверим базовую структуру
            ...
            // Проверяем, содержит ли код контракт
            const hasContract = /contract\s+\w+/.test(this.sourceCode);
            const hasInterface = /interface\s+\w+/.test(this.sourceCode);
            const hasLibrary = /library\s+\w+/.test(this.sourceCode);
            
            // Если в режиме толерантности и нет явного контракта, используем скриптовый режим
            if (!hasContract && !hasInterface && !hasLibrary && this.options.TOLERANT_MODE) {
                console.log('[Parser] No contract definition found, using script mode');
                return this._parseAsScript(startTime);
            }
            ...
            // Если в режиме толерантности, возвращаем минимальную структуру
            if (this.options.TOLERANT_MODE) {
                console.log('[Parser] Falling back to script mode due to parse error');
                return this._parseAsScript(startTime);
            }
    ...
    _parseAsScript(startTime) {
        // Создаем минимальный AST для кода без контракта
        const ast = {
            type: 'SourceUnit',
            children: []
        };
        
        // Пытаемся извлечь функции из скрипта
        const lines = this.sourceCode.split('\n');
        const functions = this._extractFunctionsFromScript(lines);
        
        // Добавляем скрипт как псевдо-контракт
        const scriptContract = {
            name: 'Script',
            kind: 'contract',
            baseContracts: [],
            nodes: [],
            range: [0, this.sourceCode.length],
            functions: new Map(),
            stateVariables: new Map(),
            modifiers: new Map()
        };
        
        this.contracts.set('Script', scriptContract);
    ...
    _extractFunctionsFromScript(lines) {
            ...
            if (line.startsWith('function ') && !line.startsWith('//')) {
                ...
                if (match) {
                    ...
                    currentFunction = {
                        ...
                        startLine: i,
                        body: [],
                        params: this._extractFunctionParams(line)
                    };
                    
                    // Добавляем фиктивную функцию в парсер
                    const fullName = `Script.${currentFunction.name}`;
                    
                    const funcInfo = {
                        id: fullName,
                        name: currentFunction.name,
                        contract: 'Script',
                        astNode: { type: 'FunctionDefinition', range: [0, 0] },
                        parameters: { parameters: currentFunction.params },
                    ...
                    this.contracts.get('Script').functions.set(currentFunction.name, funcInfo);
                }
            }
            
            // Собираем тело функции
            if (inFunction && currentFunction) {
                currentFunction.body.push(lines[i]);
                
                // Считаем скобки
                braceCount += (lines[i].match(/{/g) || []).length;
                braceCount -= (lines[i].match(/}/g) || []).length;
                
                // Если все скобки закрыты
                if (braceCount <= 0 && i > functionStart) {
                    inFunction = false;
                    
                    // Анализируем тело функции на наличие уязвимостей
                    this._analyzeScriptFunction(currentFunction, lines.slice(functionStart, i + 1));
    ...
    _extractFunctionParams(line) {
        const params = [];
        const paramMatch = line.match(/function\s+\w+\((.*?)\)/);
    ...
    _analyzeScriptFunction(funcInfo, functionLines) {
        // Простой анализ для скриптовых функций
        const funcText = functionLines.join('\n').toLowerCase();
        
        funcInfo.hasExternalCalls = funcText.includes('.call') || 
                                   funcText.includes('.transfer') || 
                                   funcText.includes('.send') ||
                                   funcText.includes('delegatecall');
        
        funcInfo.hasStateChanges = funcText.includes('balance') ||
                                  funcText.includes('total') ||
                                  funcText.includes('=') && 
                                  (funcText.includes('0') || funcText.includes('++') || funcText.includes('--'));
        
        funcInfo.hasLoops = funcText.includes('for(') || 
                           funcText.includes('while(') ||
                           funcText.includes('do{');
    }
    ...
    _extractContracts(ast) {
        ...
        for (const child of ast.children) {
            if (child.type === 'ContractDefinition') {
                // ИСПРАВЛЕНИЕ: используем правильное поле для subNodes
                const subNodes = child.subNodes || child.nodes || [];
                
                const contractInfo = {
                    name: child.name,
                    kind: child.kind, // contract, interface, library
                    baseContracts: child.baseContracts || [],
                    nodes: subNodes,  // Используем исправленное значение
                    range: child.range,
                    functions: new Map(),
                    stateVariables: new Map(),
                    modifiers: new Map()
                ...
                for (const node of subNodes) {  // Используем subNodes здесь
        ...
        for (const node of nodes) {
            if (node.type === 'FunctionDefinition') {
                ...
                if (this.contracts.has(contractName)) {
                    this.contracts.get(contractName).functions.set(funcName, funcInfo);
                }
...
// ========== ГЛАВНЫЙ АНАЛИЗАТОР (ИСПРАВЛЕННЫЙ) ==========
class IndustrialSecurityScanner {
    ...
    scan() {
        ...
        try {
            ...
            this.parser = new IndustrialParser(this.sourceCode, this.options);
            const parseResult = this.parser.parse();
            
            if (!parseResult.success) {
                // Если парсинг не удался, но у нас есть функции из скриптового режима
                if (parseResult.error && parseResult.error.includes('Empty')) {
                    throw new Error('No code to analyze');
                }
                
                // Если в режиме толерантности, продолжаем с тем что есть
                if (this.options.TOLERANT_MODE && this.parser.functions.size > 0) {
                    console.log('[Scanner] Using tolerant mode with parsed functions');
                    this.results.stats.parsing = parseResult.stats || { parseTime: 0 };
                } else {
            ...
            if (this.options.ENABLE_INTERPROCEDURAL && this.parser.functions.size > 0) {
                try {
                    ...
                    console.warn(`[Scanner] Interprocedural analysis failed: ${error.message}`);
                }
            }
            
            // Шаг 3: Символическое исполнение для критических функций
            if (this.options.ENABLE_SYMBOLIC_EXECUTION && this.parser.functions.size > 0) {
                try {
                    this._runSymbolicExecution();
                } catch (error) {
                    console.warn(`[Scanner] Symbolic execution failed: ${error.message}`);
                }
            }
            ...
            if (this.options.ENABLE_TAINT_ANALYSIS && this.parser.functions.size > 0) {
                try {
                    this._runTaintAnalysis();
                } catch (error) {
                    console.warn(`[Scanner] Taint analysis failed: ${error.message}`);
                }
            }
            ...
            // Даже при ошибке, пытаемся использовать простой анализ
            const simpleResults = this._runSimpleAnalysis();
            
            return {
                success: true,  // Всегда возвращаем success для фронтенда
                results: simpleResults,
                note: `Used simple analysis due to: ${error.message}`
            };
        }
    }
    ...
    _runSimpleAnalysis() {
        ...
        const patterns = [
            ...
            },
            {
                ...
                type: 'UNCHECKED_CALL',
                severity: 'MEDIUM',
                message: 'Transfer without success check'
            },
            {
                regex: /\.send\(/g,
                type: 'UNCHECKED_CALL',
                severity: 'MEDIUM',
                message: 'Send without success check'
            },
            {
                regex: /delegatecall\(/g,
                type: 'DELEGATECALL',
                severity: 'HIGH',
                message: 'Delegatecall found - potential proxy vulnerability'
            },
            {
                regex: /selfdestruct\(/g,
                type: 'SELFDESTRUCT',
                severity: 'HIGH',
                message: 'Selfdestruct found'
            },
            {
                regex: /tx\.origin/g,
                type: 'TX_ORIGIN',
        ...
        return {
            stats: {
                ...
                functionsAnalyzed: this._countFunctions(lines),
